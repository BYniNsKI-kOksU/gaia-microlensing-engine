...
def _render_one_frame(task):
    ...
    # Ładuj cache pierwszy raz w workerze (pickle jest thread-safe, więc
    # wczytujemy raz na worker w init, ale multiprocessing używa fork/spawn,
    # więc cache jest globalny w obrębie procesu — tu wczytujemy z pliku)
    global _W_CACHE
    if _W_CACHE is None:
