...
# Model jasności pasa Drogi Mlecznej:
# maksymalna jasność w zewnętrznej części dysku,
# stopniowe tłumienie w kierunku centrum galaktycznego
b_deg = np.abs(np.degrees(LAT))
plane_weight = np.exp(-(b_deg / 12.0)**2)

# silniejsza kompresja jasności przy centrum (l=0)
l_center = np.abs(np.degrees(LON))
center_dim = 0.45 + 0.55 * (1 - np.exp(-(l_center / 35.0)**2))

# rozjaśnienie grubego pasa galaktycznego bez wypalania centrum
hist = hist * plane_weight * center_dim
