...
# Przygotowanie stałego kształtu błysku po zdefiniowaniu funkcji _sparkle_path
_default_star_path = _sparkle_path(3.0, 1.4)
microlens_core.set_paths([_default_star_path])
...
def update(frame):
    ...
    if np.any(visible):
        ...
        # Kształty gwiazd są ustawiane tylko podczas inicjalizacji
        # W animacji zmieniamy wyłącznie pozycje, rozmiary i przezroczystość
...
writer = ProgressWriter(
    FFMpegWriter(fps=ANIMATION_FPS)
)
