...
hist, lon_edges, lat_edges = np.histogram2d(    l, b,    bins=[bins_l, bins_b],    range=[[-np.pi, np.pi], [-np.pi / 2, np.pi / 2]]   # ← jawny zakres) # Wygładzeniehist = gaussian_filter(hist.T, sigma=2) # Skala logarytmiczna gęstościhist = np.log10(hist + 1) # Normalizacja względem słabszych struktur, aby centrum nie dominowałop_low = np.percentile(hist, 5)p_high = np.percentile(hist, 98.5) hist = (hist - p_low) / (p_high - p_low)hist = np.clip(hist, 0, 1) # Mocniejsza kompresja jasnego dysku i centrum,# zachowanie widoczności halo oraz zewnętrznych strukturhist = hist ** 0.55 lon_centers = (lon_edges[:-1] + lon_edges[1:]) / 2lat_centers = (lat_edges[:-1] + lat_edges[1:]) / 2LON, LAT = np.meshgrid(lon_centers, lat_centers) # ─────────────────────────────────────────────────────────────#  PALETA – przybliżenie światła widzialnego# ─────────────────────────────────────────────────────────────_colors = [    (0.00, (0.000, 0.000, 0.000)),    (0.18, (0.030, 0.025, 0.065)),    (0.38, (0.110, 0.090, 0.190)),    (0.55, (0.340, 0.290, 0.430)),    (0.70, (0.600, 0.560, 0.660)),    (0.84, (0.860, 0.840, 0.800)),    (1.00, (1.000, 0.985, 0.940)),] cmap_mw = LinearSegmentedColormap.from_list(    "milky_way_visible",    [(v, c) for v, c in _colors],    N=512)
