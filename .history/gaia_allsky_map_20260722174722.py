"""
allsky.py — generating a static sky map (Hammer projection, 16K) from the Gaia catalog.
The background map is cached to PNG + layout .npz.
"""

import matplotlib
# FORCE Agg backend (pure raster, no GUI) — MUST be done
# before `import matplotlib.pyplot`. Without this, matplotlib on macOS can
# automatically select a GUI backend (e.g. MacOSX), which with scaled
# Retina/5K display multiplies the requested dpi by the screen scaling factor.
matplotlib.use("Agg")
...
# ─────────────────────────────────────────────────────────────
#  PALETTE — built once (module is loaded once in each process)
# ─────────────────────────────────────────────────────────────
_COLORS = [
    (0.000, (0.000, 0.000, 0.000)),   # cosmic black
    (0.085, (0.008, 0.012, 0.030)),   # very dark navy
    (0.180, (0.015, 0.032, 0.060)),   # cool halo
    (0.300, (0.035, 0.070, 0.110)),   # blue-gray disk
    (0.440, (0.090, 0.150, 0.185)),   # brighter structures
    (0.575, (0.180, 0.230, 0.220)),   # creamy-gray stars and dust
    (0.700, (0.360, 0.330, 0.255)),   # star-rich zone
    (0.820, (0.640, 0.520, 0.290)),   # warm golden disk
    (0.920, (0.890, 0.760, 0.470)),   # yellow-orange center
    (1.000, (0.985, 0.945, 0.835)),   # very bright core without white
]
...
def _sparkle_path(spike_ratio, minor_ratio, valley_r=1.0, n_arms=4):
    """Normalized shape of a 'spark-star' with diffraction spikes."""
    n = n_arms * 4
...
# The star shape is identical for all events and frames —
# calculated once at module import (not in each frame as in the original).
...
# ─────────────────────────────────────────────────────────────
#  BACKGROUND MAP — computed and cached to PNG + layout .npz (no conceptual
#  changes compared to the original, just extracted to a function)
# ─────────────────────────────────────────────────────────────
def build_and_save_sky_map(fig, ax, cmap_mw):
    print(f"Loading Gaia catalog: {INPUT_FILE}")
    table = Table.read(INPUT_FILE)
    print(f"Loaded: {len(table):,} records")

    sky = table.to_pandas().dropna(subset=["l", "b", "phot_g_mean_mag"])
    print(f"After filtering: {len(sky):,} stars")

    l_deg = sky["l"].values
    b_deg = sky["b"].values

    # Physical brightness from Gaia G magnitude.
    # Bright stars get higher weight than faint ones.
    flux = 10 ** (-0.4 * sky["phot_g_mean_mag"].values)

    # Limit the influence of a few extremely bright stars.
    ...
    b = np.radians(b_deg)
    # Wrap galactic longitudes to range [-pi, pi] after axis inversion,
    # so that the histogram covers the entire map.
    l = -l
    l = (l + np.pi) % (2 * np.pi) - np.pi

    # Filter only valid (finite) values
    valid = np.isfinite(l) & np.isfinite(b) & np.isfinite(flux)
    l = l[valid]
    b = b[valid]
    flux = flux[valid]

    print(f"Range l [deg]: {np.degrees(l).min():.2f} .. {np.degrees(l).max():.2f}")
    print(f"Range b [deg]: {np.degrees(b).min():.2f} .. {np.degrees(b).max():.2f}")
    print(f"Number of points: {len(l):,}")

    print("Histogram range l:", np.degrees(l).min(), np.degrees(l).max())
    print("Histogram range b:", np.degrees(b).min(), np.degrees(b).max())

    progress = tqdm(total=1, desc="Creating 2D map", unit="step")
    ...
    print(f"Nonzero histogram cells: {np.count_nonzero(hist):,} / {hist.size:,}")
    ...
    ax.annotate(
        "Galactic longitude  l", xy=(0.5, -0.07), xycoords="axes fraction",
        ha="center", va="top", fontsize=9, color="white", annotation_clip=False,
    )
    ax.annotate(
        "Galactic latitude  b", xy=(-0.055, 0.5), xycoords="axes fraction",
    ...
    # Do not use tight_layout for Hammer projection on very large maps,
    # because it can crop part of the projection.
...
def load_cached_background(fig, ax):
    print(f"Used ready PNG map without recalculating histogram: {MAP_FILE}")
...
def main():
    ...
    plt.close(fig)
    print(f"The static sky map has been saved to file: {MAP_FILE}")
