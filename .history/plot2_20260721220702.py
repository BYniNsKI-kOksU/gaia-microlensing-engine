...
# GAUSSIAN GLOW
# -----------------------------
def gaussian_glow(ax, x, y, size, strength):

    layers = 12
    for i in range(layers, 0, -1):
        radius = size * (i / layers)
        sigma = i / layers
        alpha = strength * (1 - i / (layers + 2))**4
...
def render_frame(task):
    ...
    # Tło Drogi Mlecznej na zwykłej osi prostokątnej (imshow nie działa
    # poprawnie na osiach projekcji Hammer — współrzędne są zniekształcone)
    bg_ax = fig.add_axes([0, 0, 1, 1], zorder=0)
    bg_ax.imshow(bg, aspect="auto", origin="upper")
    bg_ax.axis("off")

    # Oś Hammer przezroczysta — nakłada siatkę i zdarzenia na tło
    ax = fig.add_subplot(111, projection="hammer", zorder=1)
    ...
    if np.any(visible):
        ...
        size = 10 + 2500 * np.log1p(strength)
...
def main():
    ...
    if not PNG_CACHE.exists():
        ...
        ax_png.set_facecolor("black")
        ax_png.grid(
            color="gray",
            alpha=0.35,
            linewidth=0.5
        )
        ax_png.set_xticklabels([])
        ax_png.set_yticklabels([])
        ...
        if np.any(visible_png):
            ...
            size_png = 10 + 500 * np.log1p(strength_png)
