...
def _init_worker(animation_dpi):
    ...
    outer = ax.scatter([], [], s=[], marker="o", edgecolors="none", zorder=100, transform=ax.transData)
    inner = ax.scatter([], [], s=[], marker="o", edgecolors="none", zorder=101, transform=ax.transData)
    core = ax.scatter([], [], s=[], marker="o", edgecolors="none", zorder=102, transform=ax.transData)
...
def _render_one_frame(task):
    ...
    if entry is None:
        ...
        _W_CORE.set_offsets(entry["offsets"])
        _W_CORE.set_sizes(entry["core_size"])
        _W_CORE.set_facecolors(entry["core_rgba"])
        _W_CORE.set_alpha(1.0)
        _W_OUTER.set_alpha(None)
        _W_INNER.set_alpha(None)
        _W_CORE.set_alpha(None)
