...
# ─────────────────────────────────────────────────────────────
_colors = [
    (0.00, (0.000, 0.000, 0.005)),
    (0.15, (0.008, 0.012, 0.035)),
    (0.30, (0.025, 0.040, 0.090)),
    (0.45, (0.080, 0.110, 0.180)),
    (0.60, (0.220, 0.190, 0.260)),
    (0.75, (0.520, 0.430, 0.350)),
    (0.88, (0.850, 0.700, 0.520)),
    (1.00, (1.000, 0.900, 0.720)),
]
...
fig = plt.figure(figsize=(32, 18), dpi=300, facecolor="black")
...
# Stała długość animacji: 30 sekund
# Każde zdarzenie jest rozciągnięte indywidualnie względem własnego tE

frames = 750
animation_duration = 30.0

# czas względny w sekundach filmu
...
def update(frame):
    ...
    # czas względny do maksimum dla każdego zdarzenia
    t_rel = current_times[frame]

    # normalizacja czasu względem własnego czasu Einsteina
    # każde zjawisko ma własną długość błysku
    t = ml_tmax + t_rel * ml_te / 14.0
    ...
    # widoczne tylko aktywne zdarzenia
    visible = amp > 1.001

    if np.any(visible):
        sizes = (amp[visible] - 1) * 350
        colors = amp[visible]
