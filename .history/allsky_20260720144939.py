...
plot_progress.update(1)
 ...
 # Przygotowanie siatki współrzędnych przed adaptacyjnym wygładzaniem
lon_centers = (lon_edges[:-1] + lon_edges[1:]) / 2
lat_centers = (lat_edges[:-1] + lat_edges[1:]) / 2
LON, LAT = np.meshgrid(lon_centers, lat_centers)

# Adaptacyjne wygładzenie: centrum Galaktyki zachowuje szczegóły,
# a słabsze obszary zewnętrzne są mocniej wygładzane
hist = hist.T

 # Waga centrum: zachowanie szczegółów jądra Galaktyki
center_weight = np.exp(-((LON / 0.9)**2 + (LAT / 0.45)**2))

hist_center = gaussian_filter(hist, sigma=1.5)
hist_outer = gaussian_filter(hist, sigma=2.5)

# Centrum pozostaje ostre, ale ramiona spiralne nie są nadmiernie wygaszane
hist = center_weight * hist_center + (1 - center_weight) * hist_outer

# Delikatne podniesienie słabych struktur dysku galaktycznego
outer_boost = 0.55 * (1 - center_weight)
hist = hist + outer_boost * np.percentile(hist, 99)
...
p_high = np.percentile(hist, 98miale.5)
...
# ─────────────────────────────────────────────────────────────
_colors = [
    # prawie czarne tło — obszary o małej liczbie gwiazd
    (0.00, (0.000, 0.000, 0.004)),
    # bardzo słabe halo gwiazdowe
    (0.12, (0.018, 0.025, 0.040)),
    # chłodne szaroniebieskie obrzeża Drogi Mlecznej
    (0.25, (0.055, 0.075, 0.110)),
    # słaby dysk galaktyczny
    (0.40, (0.110, 0.135, 0.165)),
    # gęstsze obszary ramion spiralnych
    (0.55, (0.220, 0.240, 0.260)),
    # jasny dysk widzialny
    (0.70, (0.500, 0.500, 0.480)),
    # okolice centrum galaktycznego
    (0.85, (0.780, 0.680, 0.480)),
    # zgrubienie centralne — żółto-kremowe gwiazdy starej populacji
    (1.00, (1.000, 0.900, 0.700)),
]
