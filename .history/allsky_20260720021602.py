...
# Wygładzenie - zachowanie szczegółów ramion i halo
hist = gaussian_filter(hist.T, sigma=1.2)

# Skala logarytmiczna gęstości
hist = np.log10(hist + 1)

# Kompresja dynamiki jasności, aby centrum nie było przesycone
p_low = np.percentile(hist, 8)
p_high = np.percentile(hist, 99.7)

hist = (hist - p_low) / (p_high - p_low)
hist = np.clip(hist, 0, 1)

# Przyciemnienie ekstremalnie jasnych obszarów
# i zachowanie słabych struktur halo
hist = hist ** 1.35
