...
# any spatial correlation, so dividing the values into
...
def _run_query_with_timeout(query, timeout):
    ...
    any timeout). If the server queue is congested,
    this single line can hang for hours – and then the existing
    except block never triggers, because nothing raises an exception.
    Therefore, we run the query in a separate thread (daemon, so it won't
    block program shutdown) and impose a hard timeout ourselves
    via t.join(timeout).
    """
    box = {}
...
def download_gaia():
    ...
    for i in range(N_CHUNKS):
        ...
        # is expensive and not needed here — random_index is already
        # random, and the sample is homogeneous thanks to the WHERE filter itself.
        # ORDER BY only prolongs the query execution time (and it most likely
